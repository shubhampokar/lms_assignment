...
-- Unique node in all region (doesn't take region into consideration)
SELECT 
...
--WHERE customer_id=1
--ORDER BY customer_id, region_id, start_date
--GO

-- when same node is reassign consecutively then second node data is eliminated 
-- for example, customer_id = 1 has first 2 node_id = 4, therefore second node (start_date = 2020-01-04) isn't consider during calculations
-- hence first change date difference = (2020-01-15) - (2020-01-02) = 13
WITH prev_cte AS (
	SELECT
		customer_id,
		node_id,
	...
	WHERE 
		DATEPART(YEAR, end_date) != 9999
),
day_diff_cte AS (
	SELECT 
		DATEDIFF(DAY, LAG(start_date) OVER (PARTITION BY customer_id ORDER BY start_date), start_date) AS "day_diff"
	FROM prev_cte
	WHERE node_id != prev_node
		OR prev_node IS NULL               
)
SELECT
	ROUND(AVG(CAST(day_diff AS REAL)), 2) AS "avg_day_for_reallocation",
	ROUND(AVG(CAST(day_diff AS REAL)), 0) AS "avg_day_for_reallocation (rounded to decimal zero)"
FROM day_diff_cte;
GO
...
SELECT
	distinct
	region_id,
	PERCENTILE_CONT(0.5) WITHIN GROUP(ORDER BY day_diff) OVER(PARTITION BY region_id) AS "cont_median",
	PERCENTILE_DISC(0.5) WITHIN GROUP(ORDER BY day_diff) OVER(PARTITION BY region_id) AS "disc_median",
	PERCENTILE_CONT(0.8) WITHIN GROUP(ORDER BY day_diff) OVER(PARTITION BY region_id) AS "cont_80_precentile",
	PERCENTILE_DISC(0.8) WITHIN GROUP(ORDER BY day_diff) OVER(PARTITION BY region_id) AS "disc_80_precentile",
	PERCENTILE_CONT(0.95) WITHIN GROUP(ORDER BY day_diff) OVER(PARTITION BY region_id) AS "continuous_95_precentile",
	PERCENTILE_DISC(0.95) WITHIN GROUP(ORDER BY day_diff) OVER(PARTITION BY region_id) AS "discrete_95_precentile"
FROM day_diff_cte;
GO
...
WITH avg_deposit_txn_cte AS (
	SELECT
		customer_id,
		COUNT(txn_amount) AS "deposit_count",
		AVG(CAST(txn_amount AS REAL)) AS "avg_deposit"
	FROM [data_bank].[customer_transactions]
...
SELECT
	...
	ROUND(AVG(avg_deposit), 2) AS "avg_deposit_per_customer"
FROM avg_deposit_txn_cte;
GO
...
valid_cte AS (
	SELECT
		...
		customer_id,
		SUM(CASE 
			WHEN (txn_type='deposit' AND txn_count > 1) THEN 1
			ELSE 0
		END) AS "dep_valid",
		SUM(CASE 
			WHEN (txn_type IN ('purchase', 'withdrawal') AND txn_count > 0) THEN 1
			ELSE 0
		END) AS "pur_with_valid"
	FROM txn_period_count_cte
	WHERE (txn_type='deposit' AND txn_count > 1) OR (txn_type IN ('purchase', 'withdrawal') AND txn_count > 0)
...
SELECT 
	...
	COUNT(DISTINCT customer_id) AS "total_customer"
FROM valid_cte
WHERE dep_valid >= 1 AND pur_with_valid >= 1
GROUP BY txn_month
GO
...
-- method 1 -- relative cost: 72%
WITH txn_period_balance_cte AS (
	SELECT 
		customer_id,
		txn_type,
...
SELECT
	...
	txn_month,
	txn_year,
	SUM(total_txn_amount) OVER(PARTITION BY customer_id ORDER BY txn_month, txn_year) AS "closing_balance"
FROM txn_period_balance_cte
GO

-- method 2 -- relative cost: 28%
WITH monthly_closing_balance_cte AS (
	SELECT 
		customer_id,
		DATEPART(MONTH, txn_date) AS "txn_month",
		DATEPART(YEAR, txn_date) AS "txn_year",
	...
	FROM [data_bank].[customer_transactions]
	GROUP BY customer_id, DATEPART(MONTH, txn_date), DATEPART(YEAR, txn_date)
)
SELECT
	customer_id,
	txn_month,
	txn_year,
	SUM(total_txn_amount) OVER(PARTITION BY customer_id ORDER BY txn_month, txn_year) AS "closing_balance"
FROM monthly_closing_balance_cte
GO
...
-- ambiguous question as there is no mention of month, are we talking about last month growth or month doesn't matter
-- or are we talking about more than 5% growth per month, i.e. grouping each data by month...

-- below query is for all those customer who have increase their closing balance wrt to their previous month...
-- that is, let say customer A increases its closing balance by 10% in March compare to Feb then he/she will be considered in final calculation even if its last month growth rate is less than 5% (here, last month is April)
WITH monthly_closing_balance_cte AS (
	SELECT 
		customer_id,
		DATEPART(MONTH, txn_date) AS "txn_month",
...
change_percentage_cte AS (
	SELECT
		*,
		LEAD(closing_balance) OVER(PARTITION BY customer_id ORDER BY txn_month, txn_year) - closing_balance AS "num",
		COALESCE(NULLIF(closing_balance, 0), POWER(2,30)) AS "div",
		(
			CAST(
				LEAD(closing_balance) OVER(PARTITION BY customer_id ORDER BY txn_month, txn_year) - closing_balance
			AS REAL)
			/
			ABS(COALESCE(NULLIF(closing_balance, 0), POWER(2,30)))
		) * 100 AS "change_perc"
	FROM customer_closing_bal_cte 
)
SELECT
	(
		(COUNT(DISTINCT customer_id) / (SELECT CAST(COUNT(DISTINCT customer_id) AS REAL) FROM [data_bank].[customer_transactions]))
	) * 100 AS "customer (%)"
FROM change_percentage_cte
WHERE change_perc>5
GO
...
-- Ans. Option 1: monthly_closing_balance - lag value of previous month
--		Option 2: monthly_avg_running_balance - lag value of previous month
--		Option 3: running_balance
		...
		*,
		...
		SUM(
			CASE
				...
				ELSE -txn_amount
			END 
		) OVER(PARTITION BY customer_id, DATEPART(MONTH, txn_date) ORDER BY txn_date ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS "monthly_closing_amount"
	FROM [data_bank].[customer_transactions]
)
SELECT
	*,
	MIN(running_balance) OVER(PARTITION BY customer_id, DATEPART(MONTH, txn_date) ORDER BY txn_date ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS "monthly_min_running_bal",
	MAX(running_balance) OVER(PARTITION BY customer_id, DATEPART(MONTH, txn_date) ORDER BY txn_date ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS "monthly_max_running_bal",
	AVG(running_balance) OVER(PARTITION BY customer_id, DATEPART(MONTH, txn_date) ORDER BY txn_date ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS "monthly_avg_running_bal"
...
---Data Bank wants to try another option which is a bit more difficult to implement - they want to calculate data growth using an interest calculation, just like in a traditional 
---savings account you might have with a bank.

---If the annual interest rate is set at 6% and the Data Bank team wants to reward its customers by increasing their data allocation based off the interest calculated on a daily 
---basis at the end of each day, how much data would be required for this option on a monthly basis?

---Special notes:
---		Data Bank wants an initial calculation which does not allow for compounding interest, however they may also be interested in a daily compounding interest calculation so you can 
---		try to perform this calculation if you have the stamina!



-- below query is for simple interest at 6% annual rate
SELECT
	*,
	SUM(
		CASE
	...
	(CAST(6 AS REAL)/365)*SUM(
		CASE
			WHEN txn_type = 'deposit' THEN txn_amount
			ELSE -txn_amount
		END 
	) OVER(PARTITION BY customer_id ORDER BY txn_date ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS "running_balance_interest"
FROM [data_bank].[customer_transactions]
GO
...
---------------------------------------------------------------------------------------------> Extension Request <---------------------------------------------------------------------------------------------
---The Data Bank team wants you to use the outputs generated from the above sections to create a quick Powerpoint presentation which will be used as marketing materials for both external 
---investors who might want to buy Data Bank shares and new prospective customers who might want to bank with Data Bank.

---> 1)	Using the outputs generated from the customer node questions, generate a few headline insights which Data Bank might use to market it’s world-leading security features to potential
---		investors and customers.

-- Ans. Headline 1: 5 UNIQUE NODES per region to store data in distributed manner to DECREASE TRAFFIC on single node.
--		Headline 2: On an average DATA IS REALLOCATED to new node in 19 DAYS to PREVENT from HACKING.
--		Headline 3: Each region has around 100 customer - AUSTRALIA is LEADING REGION with total customer count 110.





---> 2) With the transaction analysis - prepare a 1 page presentation slide which contains all the relevant information about the various options for the data provisioning so 
---		the Data Bank management team can make an informed decision.